...
-- Drop all tables in reverse dependency order (ignore errors on non-existent)
DROP TABLE IF EXISTS wrap_reflections CASCADE;
DROP TABLE IF EXISTS daily_log_summaries CASCADE;
DROP TABLE IF EXISTS app_usage_stats CASCADE;
DROP TABLE IF EXISTS gym_routine_exercises CASCADE;
DROP TABLE IF EXISTS gym_routines CASCADE;
...
DROP TABLE IF EXISTS checkin_schedule CASCADE;
...
DROP TABLE IF EXISTS bare_minimum_streak CASCADE;
DROP TABLE IF EXISTS streaks CASCADE;
DROP TABLE IF EXISTS bad_habit_logs CASCADE;
DROP TABLE IF EXISTS daily_logs CASCADE;
DROP TABLE IF EXISTS habit_levels CASCADE;
...
DROP TYPE IF EXISTS habit_stack CASCADE;
DROP TYPE IF EXISTS log_status CASCADE;
DROP TYPE IF EXISTS sprint_intensity CASCADE;
DROP TYPE IF EXISTS sprint_status CASCADE;
DROP TYPE IF EXISTS target_period CASCADE;
...
-- ENUMS
CREATE TYPE habit_category AS ENUM ('binary', 'measured', 'bad', 'manual-skill');
...
-- SYNC TABLES (from migration 002)
...
-- ============================================================
-- RLS POLICIES — user-scoped (auth.uid())
-- ============================================================
...
-- Streaks (FIXED — correct IDs for all 7 bare minimum habits)
